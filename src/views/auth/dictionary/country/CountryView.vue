<template>
    <div>
        ...
        <teleport to="body">
            <CountryDetailView :visible="visible" :data="record" @toggle="handleToggle" />
        </teleport>
    </div>
</template>
...
import CountryDetailView from './CountryDetailView.vue'
...
export default defineComponent({
    ...
    setup() {
        ...
        const items = ref<CountryModel[]>([]);
        const record = ref<CountryModel>();
        ...
        // sửa
        const handleEdit = (item: CountryModel) => {
            show(true, item);
        }

        // xóa
        const handleDelete = (item: CountryModel) => {
        ...
        const show = (v: boolean, r: CountryModel | undefined) => {
            record.value = r;
            visible.value = v;
        }
