...
export default defineComponent({
    ...
    props: {
        ...
        data: {
            type: Object as PropType<HospitalModel>
        }
    },
    setup(props, { emit }) {
        const title = ref<string>('Thêm mới bệnh viện');
        const item = ref<HospitalModel>({
            id: undefined,
            code: "",
            name: "",
            description: "",
        ...
        const handleSave = function () {
            loading.value = true;
            hospitalService.createOrEdit(item.value)
                .then(res => {
                    if (res) {
                        result = true;
                        toggle();
        ...
        watch(show, (value) => {
            if (value) {
                ...
                if (props.data !== null && props.data?.id !== undefined) {
                    let data = props.data!;
                    hospitalService.getById(data.id!)
                        .then(res => {
