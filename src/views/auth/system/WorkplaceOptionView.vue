    ...
    >
        ...
        <div class="row">
            <a-table
                ...
                :pagination="false"
                :row-selection="{
                    selectedRowKeys: roomSelectedRowKeys,
...
export default defineComponent({
    ...
    setup() {
        ...
        const branchValue = ref<string | null>();
        const departmentValue = ref<string | null>();
        ...
        const disableBranch = computed(
            () => state.loading || branchs.value.length < 2
        );
        const disableDepartment = computed(
            () => state.loading || departments.value.length < 2
        );

        const handleLoad = async () => {
            await branchService.getAll().then((response) => {
                branchs.value = response.data.result.filter((x) => !x.inactive);
            });

            if (branchs.value.length > 0) {
                let branch = branchs.value.find(
                    (x) => x.id === store.workplace.branch
                );
                branchValue.value =
                    branch !== undefined ? branch.id : branchs.value[0].id;

                await departmentService.getAll().then((response) => {
                    departments.value = response.data.result.filter(
                        (x) => !x.inactive && x.branchId === branchValue.value
                    );
                });

                if (departments.value.length > 0) {
                    let department = departments.value.find(
                        (x) => x.id === store.workplace.department
                    );
                    ...
                    await roomService.getAll().then((response) => {
                        rooms.value = response.data.result.filter(
                            (x) =>
                                !x.inactive &&
                                x.departmentId === departmentValue.value
                        );
                        let selectRowKeys = [] as string[];
                        for (var i = 0; i < rooms.value.length; i++) {
                            let room = store.workplace.rooms.find(
                                (f) => f === rooms.value[i].id
                            );
                            if (room !== undefined) {
